...
def preprocessor(ligand_dir,data_dir):
    ...
    if m1 is not None: #docking ligand file may be 0 size
            dst_dir = os.path.join(data_dir,file_flag)
            print(dst_dir)
            if not os.path.exists(dst_dir):
                print('not exist dst dir ,amke it!')
                # dst_dir = os.path.join(data_dir,file_flag)
                os.mkdir(dst_dir)
                ligand_fn = os.path.join(ligand_dir, f'{file_flag}_ligand.sdf')
                print('ligand file ',ligand_fn)
                # dst_dir = os.path.join(data_dir,file_flag)
                os.system(f'cp {ligand_fn} {dst_dir}')
                # os.mkdir(os.path.join(data_dir,file_flag),exist_ok=True)

                if len(m1.GetConformers())==0:
                    print(f"{file_flag} mol no conformer!")
                    return -1
                try:
                    pdb_path = os.path.join(ligand_dir, f'{file_flag}_protein.pdb')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--PDBbind_path", help="file path for save compounds from docking result.", type=str, \
        default='/home/house/caoduanhua/DeepLearningForDock/datasets/dockingModelTestDataset/astex_diverse_set',required=False)
    ...
    parser.add_argument("--save_dir", help="save pocket file dir.", type=str,default='/home/house/caoduanhua/DeepLearningForDock/datasets/equibind_and_diffdock_dataset/PDBBIND/astex_diverse_set_8A',required=False)
    # parser.add_argument("--prefix", help="Anything that helps you distinguish between compounds.", type=str,default='')
    parser.add_argument("--process_num", help="process num for multi process ", type=int,default=20)
