...
def preprocessor(ligand_dir):
    ...
    # try:
        # print(os.path.join(ligand_dir, f'{file_flag}_ligand.sdf'))
    m1 = read_molecule(os.path.join(ligand_dir, f'{file_flag}_prot',f'{file_flag}_l.sdf'), remove_hs=True, sanitize=True)

    if m1 is None:  # read mol2 file if sdf file cannot be sanitized
        m1 = read_molecule(os.path.join(ligand_dir,  f'{file_flag}_prot',f'{file_flag}_l.sdf'), remove_hs=True, sanitize=False)
        print("read mol fail,try not sanitize!")
        if m1 is None:
            print('Using the .sdf file failed. ')
            return -1
    ...
    if m1 is not None: #docking ligand file may be 0 size
            if not os.path.exists(os.path.join(ligand_dir, f'{file_flag}_prot',f'{file_flag}_pocket.pdb')):
                ...
                try:
                    # print(recptor_dir)
                    pdb_path = os.path.join(ligand_dir, f'{file_flag}_prot', f'{file_flag}_p.pdb')
                    # print(pdb_path)
                ...
                if len(m2.GetConformers())==0:
                    ...
                    return -1
                # save pdb pocket
                Chem.MolToPDBFile(m2,os.path.join(ligand_dir, f'{file_flag}_prot',f'{file_flag}_pocket.pdb'))
                print(f'sucessed! {file_flag}')
...
def get_pocket_with_water(complex_sample):
    status=preprocessor(complex_sample)
    # print(status)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_path", help="file path ", type=str, \
        default='/home/house/caoduanhua/score_function/data/DEKOIS_2.0x/',required=False)
    parser.add_argument("--process_num", help="process num for multi process ", type=int,default=30)
    args = parser.parse_args()

    total_dirs = [os.path.join(args.dataset_path,filename) for filename in os.listdir(args.dataset_path)]
    
    file_tuple_list = []
    for complex_sample in total_dirs:
